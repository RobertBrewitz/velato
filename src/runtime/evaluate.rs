...
pub struct EvaluatedLayer<'a> {
    ...
    pub frame: f64,
    /// Property sampling frame in the containing composition.
...
fn evaluate_layers<'a>(
    ...
    for node in &mut result {
        if let Content::Instance { name, .. } = &node.layer.content {
            // Time remapping remains unsupported, matching the renderer's existing timing.
            let child_frame =

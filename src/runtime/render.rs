...
pub trait RenderSink {
    ...
    /// Pushes a filter layer using effect parameters evaluated for the current frame.
    /// Spatial parameters are in layer coordinates, mapped by `transform`.
    ///
    /// Return [`FilterLayerResult::Pushed`] only when a layer was pushed; the caller
    /// will close it with [`Self::pop_layer`]. Return [`FilterLayerResult::Unsupported`]
    /// without changing the layer stack to skip an unsupported effect.
    ///
    /// The default implementation and the built-in Vello sinks return
    ...
    /// Optional engine emission override for the current layer group.
...
impl Renderer {
    ...
    fn render_layer(
        ...
        // Wrap effects in an outer opacity layer so opacity applies to their combined output.

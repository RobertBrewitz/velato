...
use super::{Composition, EvaluatedLayer, EvaluationError};
use kurbo::{
...
impl Renderer {
    ...
    ///
    /// Panics on hierarchy errors. Use [`Self::try_append`] to handle them explicitly.
    ///
    ...
    fn render_layer(
        ...
        for _ in 0..layer.masks.len() + (evaluated.matte.is_some() as usize * 2) {
            scene.pop_layer();
        }

...
pub fn picking_scene() -> impl FnMut(&mut Scene, &mut SceneParams<'_>) {
    ...
    move |scene, params| {
        ...
        let scene_cursor = match (params.cursor_position, params.viewport_size) {
            (Some(cursor), Some(viewport)) => {
                Some(cursor_to_scene(cursor, viewport, resolution, params.camera_transform))
            }
            _ => None,
        };
        ...
        for (i, hit) in hits.iter().enumerate() {
            ...
            scene.stroke(stroke_width, Affine::IDENTITY, stroke_color, None, &hit.rect);
...
/// Convert cursor from window coordinates to scene coordinates.
/// Inverts the full camera + auto-scale transform.
...
/// Pick the visually top-most layer containing the cursor.
/// Higher `paint_order` = rendered later = visually on top.
...
/// Each layer group occupies its own depth band of this size, ensuring clear
/// z-depth separation between groups.
...
/// One layer's axis-aligned bounding box for the current frame, together with
/// its visual stacking position.
struct LayerHit {
    name: String,
    rect: Rect,
    /// Monotonic counter incremented for every `begin_layer_group` call.
    /// We use this instead of the `layer_index` parameter because precomp layers
    /// all receive index 0 — so only this counter reflects the true paint order.
    /// Higher values mean the layer was painted later (visually on top).
    paint_order: usize,
}
...
impl LayerBoundsTracker {
    ...
    fn end_layer_group(&mut self) {
        ...
        if let Some(name) = self.current_layer.take() {
            if let Some(rect) = self.current_bounds.take() {
